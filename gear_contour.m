...
% Last Modified by GUIDE v2.5 29-Sep-2021 17:00:53
...
tp=pi*d/(2*n);
for k=1:n1;
	r=do/2-(do-db)*(k-1)/(2*(n1-1));
	pha=acos(db/(2*r));
	t=2*r*(tp/d+(tan(phi)-phi)-(tan(pha)-pha));	%tooth tickness at any angle phi
																%involute equation - 
																%refer to Shigley's book
	theta(k)=t/(2*r);
	xp(k)=r*sin(theta(k));		%change from polar coordinates to cartesian coordinates
	yp(k)=r*cos(theta(k));
end
xp=xp';yp=yp';
% ----------------------------------------------------------------------------
...
n2=5;
for k=1:n2;
	theta_o=theta(1)*(k-1)/(n2-1);
	xo(k)=(do/2)*sin(theta_o);
	yo(k)=(do/2)*cos(theta_o);
end
xo=xo';yo=yo';
% ----------------------------------------------------------------------------
%To calculate the non-involute portion of the curve- between the base circle and 
...
%to the last point of the involute curve.
	xr(k)=xp(1,10);
	yr(k)=yp(1,10)-(yp(1,10)-r_fillet-(dr/2)*cos(theta0))*k/3;
end
xr=xr';yr=yr';
% ----------------------------------------------------------------------------
%To calculate the dedendum circle
n3=5;
for k=1:n3;
   thetar=theta0+(pi/n-theta0)*(k-1)/(n3-1);	
   %(pi/n-theta0) angle subtended for dededem arc
   xro(k)=dr*sin(thetar)/2;	%xro(1) is the beginning point
	yro(k)=dr*cos(thetar)/2;
end
xro=xro';yro=yro';
% ----------------------------------------------------------------------------
%To calculate fillet
...
n4=5;
for k=1:n4;
   xf(k)=xro(1)-r_fillet*cos((k-1)*pi/(2*n4-2));
   yf(k)=yro(1)+r_fillet*(1-sin((k-1)*pi/(2*n4-2)));	%yf(5)=yro(1)-r_fillet*sin(4*pi/8)
end
xf=xf';yf=yf';
% ----------------------------------------------------------------------------
%To append each piece of curve to generate one-half of a tooth profile
...
ff=[-1 0;0 1]*g;		%reflection 
n5=n1+n2+n3+n4+3;
for k=1:n5;				%n4 points =n1(involute)+n2(addendum)+n4(fillet)
   						%			+3(noninvolute)+n3(dedendum)
   f(1,k)=ff(1,n5+1-k);	%reverse the order of the points, easy for plotting 
	f(2,k)=ff(2,n5+1-k);
end
h=[h,f,g];				%the whole tooth profile
h'
...
%To rotate and append the tooth to generate the gear
for k=1:n;
	kk=[cos(2*pi*(k-1)/n) sin(2*pi*(k-1)/n);-sin(2*pi*(k-1)/n) cos(2*pi*(k-1)/n)];
  												 		%rotation matrix
	mm=kk*h;		%rotate
	M=[M,mm];	%append
end
