...
if get(handles.radiobutton1,'value')==1
    Z{6,2}=Z{1,1}*Z{6,1}/Z{1,2}; %velocidad angular de la corona
    Z{1,4}=Z{1,1}/Z{2,1}; %d=N/P, diametro del piñon= N dientes / Paso diametral
    Z{2,4}=pi*Z{1,4}*Z{6,1}/12; %vel tangencial (pies/min)
    Z{3,4}=(33000*Z{5,1}/Z{2,4})/(cosd(20)*cosd(Z{4,1})); %W=(33000*H/V)/(cos(20)*cos(psi))
    Z{4,4}=33000*Z{5,1}/Z{2,4}; %Wt=33000*H/V - tangencial
    Z{5,4}=(33000*Z{5,1}/Z{2,4})*tand(20)/cosd(Z{4,1}); %Wr - radial
    Z{6,4}=(33000*Z{5,1}/Z{2,4})*tand(Z{4,1}); %Wa - axial
    F{3,1}=Z{5,4};F{3,2}=Z{4,4};F{3,3}=Z{4,1}; % Carga
...
% n=Z{1,1}; % Dientes del pinion
% pd=Z{2,1}; %Paso diametral del pinion
% phi_d=20;	%The pressure angle in degrees 
% % ----------------------------------------------------------------------------
% r_fillet=0.025;		%The radius of fillet
% % ----------------------------------------------------------------------------
...
% % ----------------------------------------------------------------------------%
% %Ploteo del diente
% %plot (g(1,:),g(2,:),'-.b',xo,yo,'-r', 'linewidth',4)
% 
% plot3(M(1,:),M(2,:),zeros(size(M(2,:)))+Z{3,1}/2)
% hold on
% axis('equal')
% M=M';
% Q=atan2d(M(:,2),M(:,1));
